...
if (isset($request['insert_activitylist'])) {
    $data = $activitylist_model->insertActivityList($request);
    if($data){
...
} else if (isset($request['insertActivityListByComeActivity'])) {
        $data = $activitylist_model->insertActivityListByComeActivity($request);
        if($data){
            $response_data['data'] =$data;
            $response_data['require'] = true;
            $response_data['message'] ='สำเร็จ';
