...
if ($request["action"] == "getEvidenceActivityID") {

    $data = $evidenceactivity_model->getEvidenceActivityID($request);
    if($data){
...
}else if ($request["action"] == "getEvidenceActivityBy") {

    $data = $evidenceactivity_model->getEvidenceActivityBy($request);
    if($data){
        $response_data['data'] =$data;
        $response_data['require'] = true;
        $response_data['message'] ='สำเร็จ';
